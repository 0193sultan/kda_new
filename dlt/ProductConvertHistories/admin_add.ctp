    ...
    <div class="col-md-12">
		<div class="box box-primary">
			<div class="box-header">
				<h3 class="box-title"><i class="glyphicon glyphicon-edit"></i>  <?php echo __('Add Inventory Adjustment'); ?></h3>
			...
			<div class="box-body">		
				<?php echo $this->Form->create('InventoryAdjustment', array('role' => 'form')); ?>
					<div class="form-group">
						<?php echo $this->Form->input('status', array('type' => 'select', 'class' => 'form-control', 'empty' => '---- Select Status ----', 'options' => array(1=>'In',2=>'Out'))); ?>
					</div>
					<div class="form-group">
...
<script>		
	$(document).ready(function(){			
		...
		$('#batch_no').selectChain({
			...
			type: 'post',
			data:{'product_id': 'product_id','batch_no': 'batch_no' }
		});
		...
		$(".add_more").click(function() {	
			...
			else
			{
				...
				$.ajax({
					...
					success: function(response){						
						var obj = jQuery.parseJSON(response);
						var recRow = '<tr class="table_row" id="rowCount'+rowCount+'"><td align="center">'+rowCount+'</td><td><input type="hidden" name="selected_product_id[]" class="selected_product_id" value="'+obj.Product.id+inventory_status_id+batch_no+expire_date+'"/>'+obj.Product.name+'<input type="hidden" name="product_id[]" value="'+obj.Product.id+'"/></td><td align="center"><input type="hidden" name="inventory_status_id'+obj.Product.id+'" value="'+inventory_status_id+'">'+inventory_status_text+'</td><td align="center"><input type="hidden" name="batch_no'+obj.Product.id+'" value="'+batch_no+'">'+batch_no+'</td><td align="center"><input type="hidden" name="expire_date'+obj.Product.id+'" value="'+expire_date+'">'+expire_date+'</td><td align="center">'+obj.ChallanMeasurementUnit.name+'</td><td align="center">'+obj.ProductPrice.general_price+'</td><td align="center"><input type="hidden" name="quantity'+obj.Product.id+'" class="p_quantity" value="'+quantity+'">'+quantity+'</td><td align="center"><button class="btn btn-danger btn-xs remove" value="'+rowCount+'"><i class="fa fa-times"></i></button></td></tr>'; 
