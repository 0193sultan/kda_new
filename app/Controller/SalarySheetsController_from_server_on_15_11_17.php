...
class SalarySheetsController extends AppController {
    ...
    public function admin_index() {
        ...
        $all_emp = $this->Employee->find('all', array('conditions' => array(
                'Employee.job_status_id' => 2,
                'Employee.job_status_id' => 1
            ),
    ...
    public function salary_process($emp_data = ''/* , $salaryTypesWithTrackingKey, $salaryTypesWithValue */) {
        ...
        if ($count_data >= 1 && !empty($emp_data['salarysheets']['fiscal_year_id']) && !empty($emp_data['salarysheets']['month_id'])) {
            ...
            $get_working_day = $this->FiscalYearSetting->find('all', array('conditions' => array(
                ...
                //'limit' => 1,
                'recursive' => -1));
            //pr($get_working_day[0]['FiscalYearSetting']['working_day']);
            //die();
            ...
            foreach ($emp_data['employee_id'] as $key => $value) {
                ...
                if ($value == 1) {
                    ...
                    $emp_info[] = $this->Employee->find('all', array('conditions' => array('Employee.id' => $key,'Employee.job_status_id' => 2,'Employee.job_status_id' => 1)));
            ...
            foreach ($emp_info as $key => $val) {
                ...
                $data['EmployeeSalarySheet']['gpf'] = $this->partial_salary($emp_id, $year, $month, $val[0]['EmployeeRecovery']['gpf']);
