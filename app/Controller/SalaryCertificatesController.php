 ...
 */
	...
	public function admin_print_view($emp_id = null, $fiscal_year_id = null)
	{
		...
		$res =  $this->EmployeeSalarySheet->find('all', array('fields' => array('Employee.name as name', 'Employee.id as id', 'EmployeeSalarySheet.current_basic as current_basic', 'EmployeeSalarySheet.fiscal_year_id as fiscal_year_id', 'Employee.joining_date as joining_date', 'sum(EmployeeSalarySheet.current_basic) as current_basic_sum', 'sum(EmployeeSalarySheet.house_rent) as house_rent', 'sum(EmployeeSalarySheet.education) as education', 'sum(EmployeeSalarySheet.medical) as medical', 'sum(EmployeeSalarySheet.tiffin) as tiffin', 'sum(EmployeeSalarySheet.convance) as convance', 'sum(EmployeeSalarySheet.bycycle_loan) as bicycle_loan', 'sum(EmployeeSalarySheet.gpf) as gpf', 'sum(EmployeeSalarySheet.house_build_recovery) as house_build_recovery', 'sum(EmployeeSalarySheet.computer_loan) as computer_loan') ,'conditions' => array('EmployeeSalarySheet.employee_id' => $emp_id, 'AND' => array('EmployeeSalarySheet.fiscal_year_id' =>$fiscal_year_id, 'AND' => array($between_con) ))));
