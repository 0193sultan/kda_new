...
class MonthlyAttendanceEntriesController extends AppController {
    ...
    public function admin_add() {
        ...
        if ($this->request->is('post')) {
           // pr($this->request->data);            die();
            ...
            if ($this->MonthlyAttendanceEntry->save($this->request->data)) {
                ...
                $this->Session->setFlash(__('The Monthly attendance entry could not be saved. Please, try again.'), 'flash/error');
            }
        }
        $this->loadModel('MonthlyAttendanceEntryType');
        $monthlyAttendanceEntryTypes = $this->MonthlyAttendanceEntryType->find('list');
        $this->set(compact('monthlyAttendanceEntryTypes'));
        ...
        $this->set(compact('employees', 'monthlyAttendanceEntryTypes', 'departments', 'designations', 'fiscalYears', 'months'));
    }
...
//		$this->autoRender = false; #this prevent from search a view ........
    //}
    public function admin_get_official_attendance() {
        $this->loadModel('FiscalYearSetting');
        $fiscalYearSetting = $this->FiscalYearSetting->find('first', array('conditions' =>
            array(
                'FiscalYearSetting.fiscal_year_id' => $this->request->data['fiscal_year_id'],
                'FiscalYearSetting.month_id' => $this->request->data['month_id']
            ),
            'fields' => 'FiscalYearSetting.working_day',
            //'order' => 'GpfSubscription.id DESC',
            'limit' => 1,
            'recursive' => -1));
        //pr($fiscalYearSetting);
        echo $fiscalYearSetting['FiscalYearSetting']['working_day'];
        // die();
