...
//                    $emp_loans[] = $this->EmployeeLoan->find('all', array('conditions' => array(
//                            'EmployeeLoan.employee_id' => $key,
//                            'EmployeeLoan.recovery_year_id <=' => $emp_data['salarysheets']['fiscal_year_id'],
//                            'EmployeeLoan.recovery_start_month_id <=' => $emp_data['salarysheets']['month_id'],
...
//                        'recursive' => -1));
                    ...
                    $emp_loans[] = $this->EmployeeLoan->find('all', array(
                        ...
                        'conditions' => array('EmployeeLoan.employee_id' => $key, 'EmployeeLoan.recovery_year_id <=' => $emp_data['salarysheets']['fiscal_year_id'], 'EmployeeLoan.recovery_start_month_id <=' => $emp_data['salarysheets']['month_id'], 'EmployeeLoan.status' => 1)
                            )
                    );
                    ...
                    $checkEmpOnSalarySheet[] = $this->EmployeeSalarySheet->find('all', array(
                        ...
                        'conditions' => array('EmployeeSalarySheet.employee_id' => $key,
                            'EmployeeSalarySheet.fiscal_year_id' => $emp_data['salarysheets']['fiscal_year_id'],
                            'EmployeeSalarySheet.month_id' => $emp_data['salarysheets']['month_id'])
                            )
                    );
                }
                //
            }
            $countEmployeeLoan = count($emp_loans[0]);
...
//          die();
                ...
                if ($countEmployeeLoan > 0):
                    $j = 0;
                   // echo '-------------------';
                    foreach ($emp_loans as $loanKey => $loanval) {
                         $loanHas = count($loanval);
                       // pr($loanval);
                       // die();
                        if ($loanHas > 0):
                            ...
                            $loanRecoverydata['LoanRecovery']['loan_id'] = $loanval[0]['EmployeeLoan']['loan_id'];
                            $loanRecoverydata['LoanRecovery']['employee_id'] = $loanval[0]['EmployeeLoan']['employee_id'];
                            if ($loanval[0]['EmployeeLoan']['current_due'] != 0) {
                                if ($loanval[0]['EmployeeLoan']['current_due'] >= $loanval[0]['EmployeeLoan']['installment_amount']) {
                                    $current_due = $loanval[0]['EmployeeLoan']['current_due'] - $loanval[0]['EmployeeLoan']['installment_amount'];
                                    // echo $current_due;die();
                                    $this->EmployeeLoan->updateAll(
                                            array('EmployeeLoan.current_due' => $current_due), array('EmployeeLoan.id ' => $loanval[0]['EmployeeLoan']['id'], 'EmployeeLoan.status ' => 1)
                                    );
                                    $loanRecoverydata['LoanRecovery']['recovery_loan_amount'] = $loanval[0]['EmployeeLoan']['installment_amount'];
                                    $loanRecoverydata['LoanRecovery']['employee_loan_id'] = $loanval[0]['EmployeeLoan']['id'];
                                    ...
                                    if ($loanval[0]['EmployeeLoan']['interest_amount'] == 0) {
                                        $loanval[0]['EmployeeLoan']['interest_amount_due'] = $this->interest_calculation($loanval[0]['EmployeeLoan']);
                                        $loanval[0]['EmployeeLoan']['interest_amount'] = $this->interest_calculation($loanval[0]['EmployeeLoan']);
                                    }
                                    $temp = $loanval[0]['EmployeeLoan']['installment_amount'] + $loanval[0]['EmployeeLoan']['current_due'];
                                    $interest_amount_due = $loanval[0]['EmployeeLoan']['interest_amount'] - $temp;
                                    $current_due = 0;
                                    $this->EmployeeLoan->updateAll(
                                            array('EmployeeLoan.current_due' => $current_due, 'EmployeeLoan.interest_amount' => $loanval[0]['EmployeeLoan']['interest_amount'], 'EmployeeLoan.interest_amount_due' => $interest_amount_due), array('EmployeeLoan.id ' => $loanval[0]['EmployeeLoan']['id'], 'EmployeeLoan.status ' => 1)
                                    );
                                    $loanRecoverydata['LoanRecovery']['recovery_loan_amount'] = $loanval[0]['EmployeeLoan']['installment_amount'];
                                    $loanRecoverydata['LoanRecovery']['employee_loan_id'] = $loanval[0]['EmployeeLoan']['id'];
                                    $loanRecoverydata['LoanRecovery']['created_at'] = $this->current_datetime();
                                ...
                                if ($loanval[0]['EmployeeLoan']['interest_amount'] == 0) {
                                    $loanval[0]['EmployeeLoan']['interest_amount_due'] = $this->interest_calculation($loanval[0]['EmployeeLoan']);
                                    $loanval[0]['EmployeeLoan']['interest_amount'] = $this->interest_calculation($loanval[0]['EmployeeLoan']);
                                    $gpf['GpfSubscription']['loan_interest_amount'] = $this->interest_calculation($loanval[0]['EmployeeLoan']);
                                ...
                                // pr($loanval);die();
                                if ($loanval[0]['EmployeeLoan']['interest_amount_due'] != 0) {
                                    if ($loanval[0]['EmployeeLoan']['interest_amount_due'] >= $loanval[0]['EmployeeLoan']['installment_amount']) {
                                        $interest_amount_due = $loanval[0]['EmployeeLoan']['interest_amount_due'] - $loanval[0]['EmployeeLoan']['installment_amount'];
                                        $this->EmployeeLoan->updateAll(
                                                array('EmployeeLoan.interest_amount_due' => $interest_amount_due, 'EmployeeLoan.interest_amount' => $loanval[0]['EmployeeLoan']['interest_amount']), array('EmployeeLoan.id ' => $loanval[0]['EmployeeLoan']['id'], 'EmployeeLoan.status ' => 1)
                                        );
                                        $loanRecoverydata['LoanRecovery']['recovery_loan_amount'] = $loanval[0]['EmployeeLoan']['installment_amount'];
                                        $loanRecoverydata['LoanRecovery']['employee_loan_id'] = $loanval[0]['EmployeeLoan']['id'];
                                        $loanRecoverydata['LoanRecovery']['created_at'] = $this->current_datetime();
                                        ...
                                        $this->EmployeeLoan->updateAll(
                                                array('EmployeeLoan.interest_amount_due' => $interest_amount_due, 'EmployeeLoan.interest_amount' => $loanval[0]['EmployeeLoan']['interest_amount'], 'EmployeeLoan.status' => 0), array('EmployeeLoan.id ' => $loanval[0]['EmployeeLoan']['id'], 'EmployeeLoan.status ' => 1)
                                        );
                                        $loanRecoverydata['LoanRecovery']['recovery_loan_amount'] = $loanval[0]['EmployeeLoan']['interest_amount_due'];
                                        $loanRecoverydata['LoanRecovery']['employee_loan_id'] = $loanval[0]['EmployeeLoan']['id'];
                                        $loanRecoverydata['LoanRecovery']['created_at'] = $this->current_datetime();
                                        $loanRecoverydata[] = $loanRecoverydata;
                                        $this->LoanRecovery->saveAll($loanRecoverydata);
                            ...
                            if ($loanval[0]['EmployeeLoan']['loan_type_id'] == 8):
                                if ($loanval[0]['EmployeeLoan']['current_due'] != 0) {
                                    if ($loanval[0]['EmployeeLoan']['current_due'] >= $loanval[0]['EmployeeLoan']['installment_amount']) {
                                        $gpf['GpfSubscription']['subscription_recovery'] = 0;
                                        $gpf['GpfSubscription']['total_subscription_balance'] = $gpf['GpfSubscription']['total_subscription_balance'] + $loanval[0]['EmployeeLoan']['installment_amount'];
                                        $gpf['GpfSubscription']['loan_recovery'] = $loanval[0]['EmployeeLoan']['installment_amount'];
                                        $gpf['GpfSubscription']['loan_interest_recovery'] = 0;
                                        $gpf['GpfSubscription']['employee_loan_id'] = $loanval[0]['EmployeeLoan']['id'];
                                        ...
                                        $gpfdataExtra = [];
                                        //$this->GpfSubscription->create();
                                        $gpf['GpfSubscription']['loan_recovery'] = $loanval[0]['EmployeeLoan']['current_due'];
                                        $loan_interest_recovery = $loanval[0]['EmployeeLoan']['installment_amount'] - $loanval[0]['EmployeeLoan']['current_due'];
                                        $gpf['GpfSubscription']['total_subscription_balance'] = $gpf['GpfSubscription']['total_subscription_balance'] + $loanval[0]['EmployeeLoan']['current_due'] + $loan_interest_recovery;
                                    ...
                                    if ($loanval[0]['EmployeeLoan']['interest_amount_due'] != 0) {
                                        if ($loanval[0]['EmployeeLoan']['interest_amount_due'] >= $loanval[0]['EmployeeLoan']['installment_amount']) {
                                            $gpf['GpfSubscription']['subscription_recovery'] = 0;
                                            $gpf['GpfSubscription']['total_subscription_balance'] = $gpf['GpfSubscription']['total_subscription_balance'] + $loanval[0]['EmployeeLoan']['installment_amount'];
                                            $gpf['GpfSubscription']['loan_interest_recovery'] = $loanval[0]['EmployeeLoan']['installment_amount'];
                                            $gpf['GpfSubscription']['employee_loan_id'] = $loanval[0]['EmployeeLoan']['id'];
                                            $gpf['GpfSubscription']['loan_interest_amount'] = 0;
                                            ...
                                            $interest_amount_due = 0;
                                            $gpf['GpfSubscription']['subscription_recovery'] = 0;
                                            $gpf['GpfSubscription']['total_subscription_balance'] = $gpf['GpfSubscription']['total_subscription_balance'] + $loanval[0]['EmployeeLoan']['interest_amount_due'];
                                            $gpf['GpfSubscription']['loan_interest_recovery'] = $loanval[0]['EmployeeLoan']['interest_amount_due'];
                                            $gpf['GpfSubscription']['employee_loan_id'] = $loanval[0]['EmployeeLoan']['id'];
                                            $gpf['GpfSubscription']['loan_interest_amount'] = 0;
                                            $gpf['GpfSubscription']['sub_status'] = 3;
                                            $gpfdata[] = $gpf;
...
//};
    ...
    private function __get_installment_number($installment_date = null) {
        ...
        $installment_date = new DateTime($installment_date);
        $today = new DateTime();
        $interval = $installment_date->diff($today);
    ...
    public function cutting_employee_recovery($emp_id, $fiscal_id, $month_id, $loan_id) {
        ...
        $loanRecovery = $this->LoanRecovery->find('all', array('conditions' =>
            array(
