    ...
    <div class="col-xs-12">
		<div class="box box-primary">
                ...
                <table id="MonthlyAttendanceEntries" class="table table-bordered table-striped">
					<thead>
						<tr>
							...
							<th class="text-center"><?php echo $this->Paginator->sort('datetime'); ?></th>
			...
			<?php echo $this->Html->link($monthlyAttendanceEntry['Employee']['name'], array('controller' => 'employees', 'action' => 'view', $monthlyAttendanceEntry['Employee']['id'])); ?>
						...
						<td class="text-center"><?php echo h($monthlyAttendanceEntry['MonthlyAttendanceEntry']['datetime']); ?></td>
						<td class="text-center">
							<?php if($this->App->menu_permission('monthlyAttendanceEntries','admin_view')){ echo $this->Html->link(__('<i class="glyphicon glyphicon-eye-open"></i>'), array('action' => 'view', $monthlyAttendanceEntry['MonthlyAttendanceEntry']['id']), array('class' => 'btn btn-primary btn-xs', 'escape' => false, 'data-toggle'=>'tooltip', 'title' => 'view')); } ?>
							<?php if($this->App->menu_permission('monthlyAttendanceEntries','admin_edit')){ echo $this->Html->link(__('<i class="glyphicon glyphicon-pencil"></i>'), array('action' => 'edit', $monthlyAttendanceEntry['MonthlyAttendanceEntry']['id']), array('class' => 'btn btn-warning btn-xs', 'escape' => false, 'data-toggle'=>'tooltip', 'title' => 'edit')); } ?>
							<?php if($this->App->menu_permission('monthlyAttendanceEntries','admin_delete')){ echo $this->Form->postLink(__('<i class="glyphicon glyphicon-trash"></i>'), array('action' => 'delete', $monthlyAttendanceEntry['MonthlyAttendanceEntry']['id']), array('class' => 'btn btn-danger btn-xs', 'escape' => false, 'data-toggle'=>'tooltip', 'title' => 'delete'), __('Are you sure you want to delete # %s?', $monthlyAttendanceEntry['MonthlyAttendanceEntry']['id'])); } ?>
						</td>
					</tr>
					<?php endforeach; ?>
					</tbody>
